...
python3 -c "import json;d=json.load(open('$PFIL'));assert d['pass']['block'] and d['sha']" \
...
p['titel'] = 'Sparat via wrangler dev'
json.dump({"pass": p, "meddelande": "wrangler-dev-test", "sha": sys.argv[2]}, open(sys.argv[3], 'w'), ensure_ascii=False)
PY
R=$(curl -sS -X PUT $B/pass -H "$AUTH" -H 'Content-Type: application/json' --data-binary "@$NYFIL")
echo "$R" | python3 -c "import sys,json;d=json.load(sys.stdin);assert d['ok'] and d['pass']['uppdateradAv']=='tommy'" \
